...
class _SpecTrackingMapDataset(dataset.MapDataset[T]):
  ...
  def __init__(
    ...
    if isinstance(parent, source.SourceMapDataset):
...
class _SpecTrackingIterDataset(dataset.IterDataset[T]):
  ...
  def __init__(self, parent: dataset.IterDataset):
    super().__init__(parent)
    self.output_spec = None
  ...
  def __iter__(self) -> dataset.DatasetIterator[T]:
    ...
    return _SpecTrackingDatasetIterator(
        parent_iter, lambda spec: setattr(self, 'output_spec', spec)
    )
...
class _SpecTrackingDatasetIterator(dataset.DatasetIterator[T]):
  ...
  def __next__(self) -> T:
    result = self._parent_iter.__next__()
...
def _patch_dataset_with_spec_tracking(
  ...
  """Inserts spec tracking nodes after each transformations in the dataset.
  ...
  def _patch(ds):
    ...
    if isinstance(ds, dataset.MapDataset):
      ...
      return _SpecTrackingIterDataset(ds)
...
def _build_visualization_str(
  ...
  with _patch_dataset_with_spec_tracking(ds, mock_source_output) as tracked_ds:
    if isinstance(tracked_ds, dataset.MapDataset):
      _ = tracked_ds[0]
    else:
      _ = next(tracked_ds.__iter__())
    return _build_visualization_from_tracked_spec(tracked_ds)
